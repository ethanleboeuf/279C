...
% X-dim
d_xf = w_f/2;
I_xf = (1/12)*m_f*(w_f^2 + b_f^2) + m_f*d_xf^2; % fuel tank
...
I_xp_cm = I_xp - m_sp*(y_cm)^2;
% Total X-dim
I_x = I_xf + I_xb + 2*I_xsp + I_xp;
I_x_cm = I_xf_cm + I_xb_cm + I_xsp1_cm + I_xsp2_cm + I_xp_cm;
% Y-dim
d_yf = h_f/2;
I_yf = (1/12)*m_f*(b_f^2 + h_f^2) + m_f*d_yf^2;
I_yf_cm = I_yf - m_f*(d_yf - x_cm)^2;
d_yb = h_b/2;
I_yb = (1/12)*m_b*(b_b^2 + h_b^2) + m_b*d_yb^2;
I_yb_cm = I_yb - m_b*(d_yb - x_cm)^2;
d_ysp = h_sp/2;
I_ysp = (1/12)*m_sp*(b_sp^2 + h_sp^2) + m_sp*d_ysp^2;
I_ysp_cm = I_ysp - m_sp*(d_ysp - x_cm)^2; 
d_yp = (h_p/2) + h_b;
I_yp = (1/12)*m_p*(b_p^2 + h_p^2) + m_p*d_yp^2;
