function [state_dot] = quart_kin(t, state)
...
state_dot(1:4) = 1/2 * Omega * state(1:4) + 100*err * state(1:4);

%% Euler Equations
state_dot(5) = -wy*wz * (state(10) - state(9))/state(8);
state_dot(6) = -wx*wz * (state(4) - state(10))/state(9);
state_dot(7) = -wx*wy * (state(5) - state(8))/state(10);
